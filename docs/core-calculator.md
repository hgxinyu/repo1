...
- `flipgame/index.html`：通过“放置计算器”和“会员工具”两个选择面板提供计算器入口。
...
首页的放置计算器卡片会打开公开计算器选择面板，提供星钻交易、核心换算、殿堂 / 飞升资源计算和珍宝升级。首页另设“会员工具”卡片，把凝魂魂力、远征积分和觉醒冲榜模拟器集中在同一个会员选择面板。独立的 `flipgame/Calculators.html` 入口页已移除，各会员页面继续执行原有注册会员权限检查。
