...
接口默认使用 `deepseek-flash`（DeepSeek-V4.1-Flash）。不要额外把模型名配置成 Netlify 环境变量，避免 secrets scan 把仓库里的模型字面量误判为环境变量泄漏。

管理员账号调用时，后端会把本次回答实际使用的 `model` 和 `sources` 返回给前端，页面会显示在 AI 回复下方；普通 VIP 用户只看到回答正文。涉及具体档案、神器简称或文件名的问题，应优先命中对应档案正文；数值、层数、触发条件和机制如果没有出现在检索片段中，必须回答 `待确认`，不能用名称或相似机制补全。
