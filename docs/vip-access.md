...
用户登录后可访问注册会员页面（包括凝魂魂力和远征积分）
VIP 和 SVIP 账号可额外访问觉醒冲榜模拟器和 AI玩放置；SVIP 沿用现有每个 UTC 小时最多 10 次的限制
```
...
后台按当前页面加载用户、价格或流量，并保留本页内存中的已加载结果；每个 Admin API 仍重新校验管理员权限。角色调整需选择等级后点击“应用”，当前管理员不能在 UI 中修改自己的等级。失败和重试以模块为单位，价格未保存时切换页面保留输入，重载需确认。用户列表主邮箱使用一次联合查询，在服务端解密并掩码后返回；流量记录采用全局最多 8 个并发读取。详细行为和验证边界见 `docs/admin-performance.md`。

## 动态攻略库（2026-09-07）

主页“免费会员工具箱”保持注册会员权限；“VIP 工具箱”收纳 AI 玩放置、觉醒冲榜模拟器和 VIP 攻略。动态攻略通过 `GuideLibrary.html` 展示、`GuideAdmin.html` 发布，后台入口位于 Admin 左侧。

`/api/guides` 对 VIP 阅读清单和每张原图均校验 `canAccessPremium`，VIP/SVIP/admin 可读；普通/周活动已发布攻略公开可读。未发布攻略不对外提供。创建、上传、发布、下架、管理员列表需要 `isAdmin`；写入要求可信 Origin 和 `X-CSRF-Token`。VIP 原图不放在公开静态目录，返回 no-store，不能通过隐藏按钮代替服务端鉴权。部署不会自动导入本地 PDF。
