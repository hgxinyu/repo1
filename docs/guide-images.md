...
项目攻略图默认技能：[`ih-guide-images`](../skills/ih-guide-images/SKILL.md)。本次周攻略讨论的总结和案例见[9/11 对话记录](../skills/ih-guide-images/references/2026-09-11-session.md)。
...
- 每周攻略当前默认导出 1200×1600，发布前按实际尺寸检查中英文小字清晰度；用户需要更大尺寸时，再从同一排版源直接导出2400×3200。
...
- 优先在可编辑排版源中生成一处水印；已有Python生成器会自动加水印。只有源稿未带水印、且适用工具规则允许时，才在新鲜无水印图上执行 `python3 scripts/apply-guide-watermark.py <输入图> <输出图>` 一次；不要对已带水印图片重复处理。导出后必须打开中英文最终图片，以肉眼确认正文中段的水印实际可见；模板或脚本中存在水印代码不算验收通过。
...
- 最新默认：顶部背景简洁，给标题留出干净空间；正文卡片使用纯色底，不添加山水、竹叶、红枫或纹理背景。道具图标、功能色条及单处淡水印保留。用户另有明确要求时再调整。
- 推荐版式保持：深色主标题页眉、活动摘要卡、三张资源路线卡、双栏付费与避坑区、底部说明卡。每周可以替换内容，但保持阅读层级和留白节奏一致。
- 画面中部的正文区域保留一处淡色斜向 `ShineGame.Pro` 水印，横向居中、适当放大，不缩成单张卡片里的小角标。文字与关键数字置于水印上层，整行水印不应被标签等元素截断；页眉和Logo区域保持干净。
...
- 最终中文、英文图当前默认1200×1600；发布前以实际像素尺寸检查标题、正文、数字、居中水印、Logo与底部品牌的清晰度和遮挡情况。
- 生成提示词中的尺寸不等于实际输出尺寸。必须读取文件像素；不能把1086×1448等尺寸的输出称为2400×3200高清图，简单放大也不能证明细节恢复。保留确认过的文案与干净素材，避免连续整图编辑造成文字模糊及Logo漂移。
- 密集文字周图默认先确定中英文文案，再从干净素材建立可编辑排版源。插画单独生成，文字、边框、色块、水印独立排版，Logo和图标直接复用原文件。每次修改源稿并重新导出，不将上版PNG继续补字或作为英文翻译底图；流程与可运行示例见[`ih-guide-images`](../skills/ih-guide-images/SKILL.md#默认制作流程确认文案后从可编辑源稿导出)。
...
- 本周活动攻略默认按视口宽高等比缩放，整张图在一页内完整显示、不产生纵向滚动；仍可用双指放大查看细节。
...
固定来源：先查看 [周更新公告](https://discord.com/channels/266570913854521346/600387218086952977)，再查看 [周更新分析](https://discord.com/channels/266570913854521346/791624031098896404)。分析未更新时可参考历史记录，但须逐项核对当周公告与奖励图片，不把旧活动名称、档位或英雄珍宝评价直接套用。

- 成图只写“参考历史记录”，不显示旧分析的具体日期；完整来源链接与日期保留在知识笔记中。
- 适当使用来源中实际的小道具图标，搭配数字与简短资源名，避免纯文字堆叠；先确认每个图标的身份，尤其区分钻石、绑钻、星碎和宝库币。图标保持小而清晰，不用猜测图案替代真实资源。
- 活动名称与奖励档位分开写。例如 600 分 Soul-Awakening Gala 中的 100 分奖励档，不应写成一个独立的“100 分活动”；最高档也不等于人人都应该完成的目标。

1. 根据当周资料制作中文和英文两张同版式1200×1600攻略图（用户指定时再导出更大尺寸），按 `weekly-event-YYYY-MM-DD.jpg` / `weekly-event-YYYY-MM-DD-en.jpg` 保存；按实际尺寸检查正文、小字和数字清晰度。
...
7. 打开最终中英文图片，确认正文中段有且仅有一处淡色斜向 `ShineGame.Pro` 水印，并且没有遮挡关键数字。源稿已包含水印时不再后加；仅对允许后处理的无水印成图执行一次 `scripts/apply-guide-watermark.py`。
